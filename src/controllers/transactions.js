...
const callHandleTaxTransaction = async (params) => {
  try {
    const results = await db.sequelize.query('CALL HandleTaxTransaction(:query_type, :user_id, :agent_id,:org_code,:rev_code, :description, :nin_id, :org_name, :paid_by, :confirmed_by, :payer_acct_no, :payer_bank_name, :cr, :dr, :transaction_date, :transaction_type, :status, :reference_number)', {
      replacements: { ...params },
    });
    return results;
  } catch (err) {
...
export const postTrx = async (req, res) => {
  ...
  try {
    ...
    const transactionResults = await Promise.all(tax_list.map(callHandleTaxTransactionAsync));

    // Check if any transaction failed
    const hasFailedTransaction = transactionResults.some((result) => !result.success);

    if (hasFailedTransaction) {
      return res.status(500).json({ success: false, message: 'Error executing some stored procedures' });
    }
    ...
    console.error('Error executing stored procedure:', err);
    return res.status(500).json({ success: false, message: 'Error executing stored procedures' });
  }
};
...
export const getTrx = async (req, res) => {
  const {
    ...
    try {
      ...
      console.error('Error executing stored procedure:', error);
      res.status(500).json({ success: false, message: 'Error executing stored procedure' });
    }
};
