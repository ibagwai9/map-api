...
DELIMITER $$
DROP CREATE PROCEDURE IF EXISTS `TaxClearance` $$
CREATE PROCEDURE `TaxClearance`(IN `query_type` VARCHAR(100), IN `in_id` VARCHAR(100), IN `in_date_issued` DATE, IN `in_tin` VARCHAR(100), IN `in_tax_file_no` VARCHAR(100), IN `in_taxID` VARCHAR(100), IN `in_tax_payer` VARCHAR(150), IN `in_income_source` VARCHAR(100), IN `in_year` VARCHAR(10), IN `in_first_amount` DOUBLE(10,2), IN `in_second_amount` DOUBLE(10,2), IN `in_third_amount` DOUBLE(10,2), IN `in_first_year` DATE, IN `in_second_year` DATE, IN `in_third_year` DATE, IN `in_status` VARCHAR(100), IN `in_remark` VARCHAR(100), IN `in_recommendation` VARCHAR(400), IN `in_recommended_by` VARCHAR(100), IN `in_approved_by` VARCHAR(100), IN `in_printed` INT(2), IN `in_printed_by` VARCHAR(100), IN `in_staff_name` VARCHAR(100), IN `in_from` DATE, IN `in_to` DATE, IN `in_limit` INT(4), IN `in_offset` INT(4))
BEGIN
